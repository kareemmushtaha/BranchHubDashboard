...
class SessionDrinkController extends Controller
{
    /**
    ...
    {
        $this->authorize('view session drinks');
        
        $query = SessionDrink::with(['drink', 'session.user']);
        ...
        // Get only hourly users for the dropdown
        $users = User::where('status', 'active')
            ->where('user_type', 'hourly')
            ->orderBy('name')
            ->get();
        ...
        return view('session-drinks.index', compact('items', 'dateFrom', 'dateTo', 'users', 'drinks', 'totalPrice'));
    }
}
