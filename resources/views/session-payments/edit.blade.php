...
@section('content')
    ...
    <div class="card mb-4">
        ...
        <div class="card-body py-2">
            <div class="d-flex flex-wrap align-items-center justify-content-center gap-3" dir="rtl">
                <div class="d-flex align-items-center">
                    <i class="bi bi-clock text-warning ms-1"></i>
                    <span class="text-muted small ms-1">تاريخ الجلسة:</span>
                ...
                <div class="d-flex align-items-center">
                    <i class="bi bi-calendar-event text-info ms-1"></i>
                    <span class="text-muted small ms-1">نوع الجلسة:</span>
                    <span class="fw-bold">{{ $sessionPayment->session->session_category }}</span>
                </div>
                <span class="text-muted">|</span>
                <div class="d-flex align-items-center">
                    <i class="bi bi-person text-success ms-1"></i>
                    <span class="text-muted small ms-1">المستخدم:</span>
                    <span class="fw-bold">{{ $sessionPayment->session->user->name ?? 'غير محدد' }}</span>
                </div>
                <span class="text-muted">|</span>
                <div class="d-flex align-items-center">
                    <i class="bi bi-hash text-primary ms-1"></i>
                    <span class="text-muted small ms-1">رقم الجلسة:</span>
                    ...
                    @elseif($sessionPayment->session)
                        <span class="text-muted fw-bold">
                            #{{ $sessionPayment->session->id }}
                            <small class="badge bg-secondary me-1">محذوفة</small>
                        </span>
                    @else
            ...
            <form action="{{ route('session-payments.update', $sessionPayment) }}" method="POST" id="payment-form">
                ...
                <div class="row">
                    <div class="col-12 mb-3">
                        <label for="note" class="form-label pt-4">
          ملاحظة: صاحب الجلسة يتبقى له مبلغ مالي و يستحق على برانش هب استرجاع المبلغ له عندما يتوفر المبلغ                             <span id="note_required_indicator" class="text-danger d-none">*</span>
                        </label>
                        <textarea class="form-control @error('note') is-invalid @enderror"
                                  id="note" name="note" rows="3"
                                  placeholder="اكتب ملاحظة حول الدفع ( لان الزبون دفع مبلغ اكبر من المبلغ المطلوب)">{{ old('note', $sessionPayment->note) }}</textarea>
                        @error('note')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
