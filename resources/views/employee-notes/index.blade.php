...
@section('title', 'ملاحظات الموظفين')
...
<div class="row g-4">
    <div class="col-lg-8">
        <div class="card calendar-card p-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <a class="btn btn-outline-secondary btn-sm" href="{{ route('employee-notes.index', ['month' => $prevMonth->month, 'year' => $prevMonth->year]) }}">
                    <i class="bi bi-chevron-right"></i> الشهر السابق
                </a>
                <div class="text-center">
                    <h4 class="mb-0">{{ $currentMonthLabel }}</h4>
                    <small class="text-muted">اختر اليوم لإضافة أو استعراض ملاحظات الموظفين</small>
                </div>
                <a class="btn btn-outline-secondary btn-sm" href="{{ route('employee-notes.index', ['month' => $nextMonth->month, 'year' => $nextMonth->year]) }}">
            ...
            <div class="calendar-grid">
                @foreach ($weeks as $week)
                    @foreach ($week as $day)
                        ...
                        @endphp
                        <a href="{{ route('employee-notes.index', ['month' => $currentMonth->month, 'year' => $currentMonth->year, 'date' => $dateKey]) }}"
    ...
    <div class="col-lg-4">
        <div class="notes-panel card p-4">
            <h5 class="mb-3">
                ملاحظات الموظفين - يوم {{ $selectedDate->format('Y-m-d') }}
            </h5>
            <form method="POST" action="{{ route('employee-notes.store') }}" class="mb-4">
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="content" rows="3" class="form-control" placeholder="دوّن ملاحظاتك عن الموظف" required></textarea>
                </div>
            ...
            <div class="notes-list">
                    ...
                    <div class="card mb-3 border-0 shadow-sm">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-start">
                                <div>
                                    <h6 class="mb-1">
                                        <span class="badge bg-info me-2">{{ $note->employee_name }}</span>
                                ...
                                @if ($canModifyNote)
                                    <div class="ms-2">
                                        ...
                                        <form action="{{ route('employee-notes.destroy', $note) }}" method="POST" class="d-inline" onsubmit="return confirm('هل أنت متأكد من حذف هذه الملاحظة؟');">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @if ($canModifyNote)
                                <div class="collapse mt-3" id="edit-note-{{ $note->id }}">
                                    <form action="{{ route('employee-notes.update', $note) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="mb-2">
                                            <label class="form-label small">اسم الموظف <span class="text-danger">*</span></label>
                                            <input type="text" name="employee_name" class="form-control" value="{{ $note->employee_name }}" required>
                                        </div>
                                        <div class="mb-2">
                                        ...
                                        <div class="mb-2">
                                            <label class="form-label small">المحتوى <span class="text-danger">*</span></label>
                ...
                @empty
                    <div class="text-center text-muted">
                        لا توجد ملاحظات للموظفين لهذا اليوم بعد.
                    </div>
                @endforelse
            </div>
        </div>
